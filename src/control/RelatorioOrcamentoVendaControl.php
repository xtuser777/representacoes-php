...
class RelatorioOrcamentoVendaControl
{
    public function obter(int $ordem)
    {
        ...
        $orcamentos = OrcamentoVenda::findAll($this->traduzirOrdem($ordem));
    ...
    public function obterPorCliente(int $cliente, int $ordem)
    {
        ...
        $orcamentos = OrcamentoVenda::findByClient($cliente, $this->traduzirOrdem($ordem));
    ...
    public function obterPorFiltro(string $filtro, int $ordem)
    {
        ...
        $orcamentos = OrcamentoVenda::findByFilter($filtro, $this->traduzirOrdem($ordem));
    ...
    public function obterPorFiltroCliente(string $filtro, int $cliente, int $ordem)
    {
        ...
        $orcamentos = OrcamentoVenda::findByFilterClient($filtro, $cliente, $this->traduzirOrdem($ordem));
    ...
    public function obterPorPeriodo(string $dataInicio, string $dataFim, int $ordem)
    {
        ...
        $orcamentos = OrcamentoVenda::findByPeriod($dataInicio, $dataFim, $this->traduzirOrdem($ordem));
    ...
    public function obterPorPeriodoCliente(string $dataInicio, string $dataFim, int $cliente, int $ordem)
    {
        ...
        $orcamentos = OrcamentoVenda::findByPeriodClient($dataInicio, $dataFim, $cliente, $this->traduzirOrdem($ordem));
    ...
    public function obterPorFiltroPeriodo(string $filtro, string $dataInicio, string $dataFim, int $ordem)
    {
        ...
        $orcamentos = OrcamentoVenda::findByFilterPeriod($filtro, $dataInicio, $dataFim, $this->traduzirOrdem($ordem));
    ...
    public function obterPorFiltroPeriodoCliente(string $filtro, string $dataInicio, string $dataFim, int $cliente, int $ordem)
    {
        ...
        $orcamentos = OrcamentoVenda::findByFilterPeriodClient($filtro, $dataInicio, $dataFim, $cliente, $this->traduzirOrdem($ordem));
    ...
    public function traduzirOrdem(int $ordem)
    {
        ...
        switch ($ordem) {
            case 1: $ordemTraduzida = "ov.orc_ven_descricao"; break;
            case 2: $ordemTraduzida = "ov.orc_ven_descricao DESC"; break;
            case 3: $ordemTraduzida = "ov.orc_ven_nome_cliente"; break;
            case 4: $ordemTraduzida = "ov.orc_ven_nome_cliente DESC"; break;
            case 5: $ordemTraduzida = "vpf.pf_nome"; break;
            case 6: $ordemTraduzida = "vpf.pf_nome DESC"; break;
            case 7: $ordemTraduzida = "ov.orc_ven_data"; break;
            case 8: $ordemTraduzida = "ov.orc_ven_data DESC"; break;
            case 9: $ordemTraduzida = "apf.pf_nome"; break;
            case 10: $ordemTraduzida = "apf.pf_nome DESC"; break;
            case 11: $ordemTraduzida = "ov.orc_ven_valor"; break;
            case 12: $ordemTraduzida = "ov.orc_ven_valor DESC"; break;
        }
    ...
    public function gerarRelatorioOrcamentos(string $filtro, string $inicio, string $fim, int $cliente, int $ordem): string
    {
        ...
        $tituto = "RELATÓRIO DE ORÇAMENTOS DE VENDA";
        $filtros = "";
        ...
        if ($filtro === '' && $inicio === '' && $fim === '' && $cliente === 0) {
            ...
            if ($filtro !== '' && $inicio !== '' && $fim !== '' && $cliente !== 0) {
                $orcamentos = OrcamentoVenda::findByFilterPeriodClient($filtro, $inicio, $fim, $cliente, $this->traduzirOrdem($ordem));
                ...
                if ($filtro !== '' && $inicio !== '' && $fim !== '' && $cliente === 0) {
                    $orcamentos = OrcamentoVenda::findByFilterPeriod($filtro, $inicio, $fim, $this->traduzirOrdem($ordem));
                    ...
                    if ($filtro !== '' && $inicio === '' && $fim === '' && $cliente !== 0) {
                        ...
                        if ($filtro !== '' && $inicio === '' && $fim === '' && $cliente === 0) {
                            ...
                            if ($filtro === '' && $inicio !== '' && $fim !== '' && $cliente !== 0) {
                                $orcamentos = OrcamentoVenda::findByPeriodClient($inicio, $fim, $cliente, $this->traduzirOrdem($ordem));
                                $cli = $orcamentos[0]->getCliente()->getTipo() === 1
                                    ? $orcamentos[0]->getCliente()->getPessoaFisica()->getNome()
                                    : $orcamentos[0]->getCliente()->getPessoaJuridica()->getNomeFantasia();
                                ...
                                if ($filtro === '' && $inicio !== '' && $fim !== '' && $cliente === 0) {
                                    $orcamentos = OrcamentoVenda::findByPeriod($inicio, $fim, $this->traduzirOrdem($ordem));
        ...
        $relatorio->Cell(34, 4, $col6, "B");
        $relatorio->Cell(33, 4, $col7, "B");
        $relatorio->Cell(27, 4, $col8, "B");
        $relatorio->Cell(24, 4, $col9, "B");

        $y = 46;
        $relatorio->SetFont("Arial", "", 8);
        /** @var OrcamentoVenda $orcamento */
        foreach ($orcamentos as $orcamento) {
            $cod = $orcamento->getId();
            ...
            $vdd = $orcamento->getVendedor() ? $orcamento->getVendedor()->getPessoa()->getNome() : "";
            ...
            $relatorio->Cell(34, 4, $col6);
            $relatorio->Cell(33, 4, $col7);
