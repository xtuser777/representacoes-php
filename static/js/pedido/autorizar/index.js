    ...
    $.each(dados, function () {
        ...
        txt +=
            '<tr>\
                ...
                <td><a role="button" class="glyphicon glyphicon-edit" data-toggle="tooltip" data-placement="top" title="VISUALIZAR ETAPAS" href="javascript:visualizar(' + this.id + ')"></a></td>\
            </tr>';
    });
...
async function obter(ordem = '1') {
    let res = await postJSON(
        '/representacoes/pedido/autorizar/obter.php',
        { ordem: ordem }
    );
...
$(document).ready(async function (event) {
    ...
    let status = get('/representacoes/pedido/autorizar/obter-status.php');
    ...
    if (filtro === "" && dataInicio === "" && dataFim === "" && status === 0) {
        ...
        if (filtro !== "" && dataInicio !== "" && dataFim !== "" && status > 0) {
            if (inicio > fim) {
                ...
                let res = await postJSON(
                    '/representacoes/pedido/autorizar/obter-por-filtro-periodo-status.php',
                    {
                        filtro: filtro,
                        inicio: dataInicio,
                        fim: dataFim,
            ...
            if (filtro !== "" && dataInicio === "" && dataFim === "" && status > 0) {
                let res = await postJSON(
                    '/representacoes/pedido/autorizar/obter-por-filtro-status.php',
                    {
                        filtro: filtro,
                        status: status,
                        ordem: ordem
                ...
                if (filtro === "" && dataInicio !== "" && dataFim !== "" && status > 0) {
                    if (inicio > fim) {
                        ...
                        let res = await postJSON(
                            '/representacoes/pedido/autorizar/obter-por-periodo-status.php',
                            {
                                inicio: dataInicio,
                                fim: dataFim,
                                status: status,
                    ...
                    if (filtro === "" && dataInicio === "" && dataFim === "" && status > 0) {
                        let res = await postJSON(
                            '/representacoes/pedido/autorizar/obter-por-status.php',
                            {
                                status: status,
                                ordem: ordem
                            }
                        ...
                        if (filtro !== "" && dataInicio !== "" && dataFim !== "" && status === 0) {
                            if (inicio > fim) {
                                ...
                                let res = await postJSON(
                                    '/representacoes/pedido/autorizar/obter-por-filtro-periodo.php',
                                    {
                                        filtro: filtro,
                                        inicio: dataInicio,
                                        fim: dataFim,
                            ...
                            if (filtro !== "" && dataInicio === "" && dataFim === "" && status === 0) {
                                let res = await postJSON(
                                    '/representacoes/pedido/autorizar/obter-por-filtro.php',
                                    {
                                        filtro: filtro,
                                        ordem: ordem
                                    }
                                ...
                                if (filtro === "" && dataInicio !== "" && dataFim !== "" && status === 0) {
                                    if (inicio > fim) {
                                        ...
                                        let res = await postJSON(
                                            '/representacoes/pedido/autorizar/obter-por-periodo.php',
                                            {
                                                inicio: dataInicio,
                                                fim: dataFim,
                                                ordem: ordem
...
async function visualizar(id) {
    let res = await postJSON(
        '/representacoes/pedido/autorizar/enviar.php',
        { id: id}
    );

    if (res.status) {
        window.location.href = "../../pedido/autorizar/visualizar";
    } else {
        mostraDialogo(
            `Código: ${res.error.code}. <br />
            Erro: ${res.error.message}`,
