...
const dtadm = document.getElementById("dtAdm");
...
const cbnivel = document.getElementById("cbNivel");
...
const msAdm = document.getElementById("msAdm");
const msTipo = document.getElementById("msTipo");
...
const msNivel = document.getElementById("msNivel");
...
$(document).ready(function () {
    ...
    let data = get("/representacoes/nivel/obter.php");
    if (data != null && data !== "") {
    ...
    let response = get("/representacoes/gerenciar/funcionario/detalhes/obter-detalhes.php");
...
function verificarLogin(login) {
    $.ajax({
        type: 'POST',
        url: '/representacoes/gerenciar/funcionario/detalhes/verificar-login.php',
        data: { login: login },
        async: false,
...
function verificarAdmin() {
    let data = get("/representacoes/gerenciar/funcionario/detalhes/is-last-admin.php");
...
function verificarCpf(cpf) {
    $.ajax({
        type: 'POST',
        url: '/representacoes/gerenciar/funcionario/detalhes/verificar-cpf.php',
        data: { cpf: cpf },
        async: false,
        success: function (response) {
...
btsalvar.addEventListener("click", function (event) {
    ...
    if (erros === 0) {
        ...
        $.ajax({
            type: 'POST',
            url: '/representacoes/gerenciar/funcionario/detalhes/alterar.php',
            data: form,
            contentType: false,
            processData: false,
            success: function (response) {
                if (response.length > 0) {
                    mostraDialogo(
                        "<strong>Ocorreram problemas ao gravar as alterações...</strong>" +
                        "<br/>"+response, 
                        "danger", 
                        2000
                    );
                    ...
                    mostraDialogo(
                        ...
                        "<br />A alteração feita nos campos do funcionário foram salvos com sucesso!", 
                        "success", 
                        2000
                    );
                }
